...
- `index.html` is the overview and a small working synthetic comparison.
...
The illustration's colors are decorative. The numerical comparison starts with 82 invented region IDs and 30 synthetic references. This is separate from the Neural Observatory's 68 cortical regions and 14 deep structures. No measured study values or anatomical crosswalk have been added.
...
Edit the HTML for copy and links, `site.css` for public-page presentation, or `src/workspace-theme.css` for the analysis theme. Keep calculations in the existing core modules. The homepage imports their real Spearman implementation rather than duplicating it.
